...
def train_sbert(data_dir, models_dir, epochs=10):
    start_total = time.time()
    
    # --- BUOC 1: DOC DU LIEU ---
    print_header(1, "Khoi tao va Doc du lieu nguon (SBERT)")
    ...
    print_header(2, "Lam sach van ban cho Vietnamese-SBERT")
    ...
    # --- BUOC 3: TOKENIZATION ---
    print_header(3, "Ma hoa du lieu (SBERT Tokenization)")
    model_name = "keepitreal/vietnamese-sbert"
    print(f"  - [1/3] Dang tai SBERT Tokenizer: {model_name}...")
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    
    print(f"  - [2/3] Dang chuyen doi van ban thanh chuoi Token ID...")
    ...
    # --- BUOC 4: KHOI TAO MO HINH SBERT ---
    print_header(4, "Thiet lap kien truc SBERT")
    model = AutoModelForSequenceClassification.from_pretrained(model_name, num_labels=2)
    
    output_dir = os.path.join(models_dir, "sbert_checkpoints")
    ...
    print_header(5, "Bat dau Fine-tuning Vietnamese-SBERT")
    trainer = Trainer(
        model=model,
        args=training_args,
    ...
    print_header(6, "Danh gia va Luu mo hinh SBERT")
    ...
    sbert_save_path = os.path.join(models_dir, "sbert_model")
    model.save_pretrained(sbert_save_path)
    tokenizer.save_pretrained(sbert_save_path)
    ...
    # --- BUOC 7: TONG KET ---
    print_header(7, "Hoan tat quy trinh Vietnamese-SBERT")
    print(f"  - Thoi gian huan luyen: {duration:.2f} giay")
    print("="*80 + "\n")
