...
def train_vibert(data_dir, models_dir, epochs=10):
    start_total = time.time()
    
    # --- BUOC 1: DOC DU LIEU ---
    print_header(1, "Khoi tao va Doc du lieu nguon (ViBERT)")
    ...
    print_header(2, "Lam sach van ban cho ViBERT")
    ...
    # --- BUOC 3: TOKENIZATION ---
    print_header(3, "Ma hoa du lieu (ViBERT Tokenization)")
    model_name = "FPTAI/vibert-base-cased"
    print(f"  - [1/3] Dang tai ViBERT Tokenizer: {model_name}...")
    ...
    # --- BUOC 4: KHOI TAO MO HINH VIBERT ---
    print_header(4, "Thiet lap kien truc ViBERT")
    model = AutoModelForSequenceClassification.from_pretrained(model_name, num_labels=2)
    
    output_dir = os.path.join(models_dir, "vibert_checkpoints")
    ...
    print_header(5, "Bat dau Fine-tuning ViBERT")
    trainer = Trainer(
        model=model,
        args=training_args,
    ...
    print_header(6, "Danh gia va Luu mo hinh ViBERT")
    ...
    vibert_save_path = os.path.join(models_dir, "vibert_model")
    model.save_pretrained(vibert_save_path)
    tokenizer.save_pretrained(vibert_save_path)
    ...
    # --- BUOC 7: TONG KET ---
    print_header(7, "Hoan tat quy trinh ViBERT")
